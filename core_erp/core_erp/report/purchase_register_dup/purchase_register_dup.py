...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for inv in invoice_list:
		...
		category = frappe.db.get_value("Item",inv.item_code,"category")
		sub_group = frappe.db.get_value("Item",inv.item_code,"item_sub_group")
		hsn = frappe.db.get_value("Item",inv.item_code,"gst_hsn_code")
		uom = frappe.db.get_value("Item",inv.item_code,"stock_uom")
		row = [inv.name, inv.posting_date, inv.supplier, inv.supplier_name,inv.item_code,inv.item_name,hsn,uom,category,sub_group,inv.invoice_quantity,inv.rate_s,inv.amount_s,inv.address_display]
		...
		row += [
			...
			", ".join(purchase_order),inv.transaction_date, ", ".join(purchase_receipt), inv.mrn_date,company_currency
		]
...
def get_columns(invoice_list, additional_table_columns):
	...
	columns = [
		...
		_("Item") + ":Link/Item:120",_("Item Name") + ":Data:80",_("HSN Code") + ":Link/GST HSN Code:80",_("Stock UOM") + ":Link/UOM:80",_("Category") + ":Link/Category:80",
		_("Sub Group") + ":Link/Item Sub Group:80",_("B. Qty") + ":Float:80",_("B. Rate") + ":Float:80",_("B. Amount") + ":Float:80",
		_("Supplier Adress") + ":Data:80",]
	...
	columns += [
		...
		_("Purchase Order") + ":Link/Purchase Order:100",_("PO Date") + ":Date:80",
		_("Purchase Receipt") + ":Link/Purchase Receipt:100",_("MRN Date") + ":Date:80",
		{
			"fieldname": "currency",
			"label": _("Currency"),
			"fieldtype": "Data",
...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		select
			pi.name, pi.posting_date, pi.credit_to, pi.supplier, pi.supplier_name, pi.tax_id, pi.bill_no, pi.bill_date,
			pi.remarks, pi.base_net_total, pi.base_grand_total, pi.outstanding_amount,
			pi.mode_of_payment,pit.item_code,pit.invoice_quantity,pit.amount_s,pi.address_display,pit.item_name,pi.mrn_date,pi.transaction_date,pit.rate_s {0}
		from `tabPurchase Invoice` as pi,`tabPurchase Invoice Item` as pit
		where pi.docstatus = 1  and pi.name = pit.parent %s
		order by pi.posting_date desc, pi.name desc""".format(additional_query_columns or '') % conditions, filters, as_dict=1)
