    ...
    for item in sorted(iwb_map):
        for wh in sorted(iwb_map[item]):
            if filters.get("company"):
                if wh_company[wh]==filters['company']:
                    for batch in sorted(iwb_map[item][wh]):
                        qty_dict = iwb_map[item][wh][batch]
                        if qty_dict.expiry_status > 16 and qty_dict.expiry_status < 30 and qty_dict.bal_qty > 0:
                            data.append([batch,wh_company[wh], item_map[item]["item_name"],item, item_map[item]["description"], flt(qty_dict.bal_qty, float_precision),
...
def get_columns(filters):
    ...
    columns = [_("Batch") + ":Link/Batch:100"] + [_("Company") + ":Link/Company:150"]  + [_("Item Name") + "::150"] + [_("Item") + ":Link/Item:100"] + [_("Description") + "::150"] + \
...
def wh_map():
    wh_m = {}
    temp = frappe.db.sql("""select name,company from `tabWarehouse`""",as_dict=1)
    for i in temp:
        wh_m[i['name']]=i['company']
    return wh_m
