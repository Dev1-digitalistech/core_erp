...
def execute(filters=None):
	...
	#print(temp_data[0])
	# data = []
	# # frappe.msgprint(str(temp_data))
	# for d in temp_data:
	# 	# frappe.msgprint(str(d))
		
	# 	for item in d['work_order']:
...
def get_data(filters):
	...
	d = frappe.db.sql("""SELECT company,production_plan ,name as work_order ,qty as fg_qty, produced_qty,
		production_item as fg_item,item_name as fg_name,bom_no FROM `tabWork Order` 
		where docstatus = 1 and produced_qty > 0 %s """%conditions,as_dict=1)
	# frappe.msgprint(str(d))
	# frappe.log_error(d,"data")
	dict=[]
	list=[]
	for r in d:
		# r[r['work_order']] = get_exploded_items(r['bom_no'],{},r['fg_qty'],r['fg_qty'])
		var =  get_exploded_items(r['bom_no'],{},r['fg_qty'],r['fg_qty'])
		# frappe.msgprint(str(var))
		# for items in var:
		# 	list.append(var[items]['stock_qty'])
		
		se_name = frappe.get_list('Stock Entry',{'work_order': r['work_order']},'name')
		# se_name=frappe.get_list("Stock Entry",{"work_order":"WO/GN/21-22/00218"})  

		for se in se_name:
			temp=frappe.db.sql("""select item_code,net_qty,gross_qty,item_name,sum(qty) as qty from `tabStock Entry Detail` sed, `tabStock Entry` se 
								where sed.parent=%s and
								sed.parent = se.name and 
								se.docstatus=1 and
								item_group!='Finished Goods' group by item_code""",se["name"],as_dict=1)
			# frappe.msgprint(str(temp))
			for t in temp: 
				# r[r['work_order']][t['item_code']]['stock_entry'] = se['name']
				# r[r['work_order']][t['item_code']]['consumed_qty'] = t['qty']
				# r[r['work_order']][t['item_code']]['yield'] = (t['qty']/r[r['work_order']][t['item_code']]['gross_qty'])*100 
				# r[r['work_order']][t['item_code']]['net_qty']=t['qty']
				...
				mp = {}
				mp['work_order'] = r['work_order']
				mp['production_plan']=r['production_plan']
				mp['bom_no']=r['bom_no']
				mp['fg_qty']=r['fg_qty']
				mp['produced_qty']=r['produced_qty']
				mp['company']=r['company']
				mp['item_code']=t['item_code']
				mp['item_name']=t['item_name']
				mp['stock_entry']=se['name']
				mp['fg_item']=r['fg_item']
				mp['net_qty']=t['net_qty']
				mp['stock_qty']=t['gross_qty']
				mp['consumed_qty']=t['qty']
				# mp['yield']= (t['qty']/r[r['work_order']][t['item_code']][t['stock_qty']])*100
				# mp['yield']= (t['qty']/t['gross_qty'])*100
