...
def execute(filters=None):
	...
	for d in temp_data:
		...
		for item in d[d['work_order']]:
			data.append([d['company'], d['production_plan'], d['work_order'], d['bom_no'], d['fg_item'], d['fg_qty'], d['produced_qty'], item, d[d['work_order']][item]['item_name'],
			d[d['work_order']][item]['net_qty'], d[d['work_order']][item]['stock_qty'], d[d['work_order']][item]['consumed_qty'], d[d['work_order']][item]['yield']  
			])
...
def get_columns(filters):
	...
	columns = [
		...
		{"label": "FG Item", "fieldname": "fg_item", "fieldtype": "Link", "options": "Item", "width": 150},
...
def get_data():
	d = frappe.db.sql("""SELECT w.company, w.production_plan ,w.name as work_order ,w.qty as fg_qty,w.produced_qty , w.production_item as fg_item, w.bom_no FROM `tabWork Order` w where w.production_plan is not NULL""",as_dict=1)
	for r in d:
		r[r['work_order']] = get_exploded_items(r['bom_no'],{},r['fg_qty'],r['fg_qty'])
		se_name = frappe.get_list('Stock Entry',{'work_order': 'WO/GZB/21-22/00003'},'name')
		for se in se_name:
			temp=frappe.db.sql("""select item_code,sum(qty) as qty from `tabStock Entry Detail` sed where sed.parent=%s and item_group!='Finished Goods' group by item_code""", se['name'],as_dict=1)
			for t in temp:
				r[r['work_order']][t['item_code']]['consumed_qty'] = t['qty']
				r[r['work_order']][t['item_code']]['yield'] = r[r['work_order']][t['item_code']]['net_qty']/t['qty']*100 
	return d
...
def get_exploded_items(bom,raw = {},s_q=0,n_q=0):
	...
	items = frappe.db.sql("""select bi.item_code, bi.item_name, bi.bom_no, bi.stock_uom,stock_qty,net_qty, 
	...
	for d in items:
		if d['bom_no']:
			raw = get_exploded_items(d['bom_no'],raw,(s_q*d['stock_qty']),(n_q*d['net_qty']))
