...
def get_column():
	return [
		...
		_("Supplier") + ":Link/Supplier:120",_("Bill No") + "::100" ,
...
def get_ordered_to_be_billed_data(args):
	...
	return frappe.db.sql("""
		Select
			`{parent_tab}`.name, `{parent_tab}`.status, `{parent_tab}`.{date_field}, `{parent_tab}`.{party}, `{parent_tab}`.supplier_bill_no,
		...
		where
			...
			and `{parent_tab}`.status not in ('Closed', 'Completed') and `{parent_tab}`.pi = 0 and `{parent_tab}`.supplier not in ('DFM Food Ltd. G.Noida','DFM Food Ltd. GZB')
