...
def get_columns(filters):
	...
	columns = [_("Item") + ":Link/Item:100"] + [_("Item Name") + "::150"] + [_("Description") + "::150"] + \
        [_("Store/Stage Name") + ":Link/Warehouse:100"] + [_("Batch") + ":Link/Batch:100"] + [_("Vendor Batch No") + ":Data:100"] + [_("Manufacturing Date") + ":Date:100"] + [_("Expiry Date") + ":Date:100"] + [_("Ref Lot Number") + "::100"] + [_("Opening Qty") + ":Float:90"] + \
        [_("In Qty") + ":Float:80"] + [_("Out Qty") + ":Float:80"] + [_("Balance Qty") + ":Float:90"] + [_("Transaction Type") + "::150"] + [_("MRN No") + ":Link/Purchase Receipt:150"] + \
        [_("UOM") + "::90"]
	...
	for d in sle:
		iwb_map.setdefault(d.item_code, {}).setdefault(d.warehouse, {})\
			.setdefault(d.batch_no, frappe._dict({
                                "opening_qty": 0.0, "in_qty": 0.0, "out_qty": 0.0, "bal_qty": 0.0, "vendor_batch_no": d.vendor_batch_no
                        }))
