...
erpnext.accounts.PurchaseInvoice = erpnext.buying.BuyingController.extend({
			...
			if(doc.outstanding_amount >= 0 || Math.abs(flt(doc.outstanding_amount)) < flt(doc.grand_total)) {
                               if (doc.total_rejected > 0 && !doc.rejected_qty){
                                       cur_frm.add_custom_button(__('Debit Note for Rejected Qty'),
                                               this.make_debit_note_for_rejected_qty, __('Create'));
                               }
                        }
	...
	make_debit_note: function() {
		frappe.model.open_mapped_doc({
			method: "core_erp.customizations.purchase_invoice.purchase_invoice.make_debit_note",
			frm: cur_frm
		})
	},

	make_debit_note_for_shortqty: function() {
                frappe.model.open_mapped_doc({
                        method: "core_erp.customizations.purchase_invoice.purchase_invoice.make_debit_note_for_shortqty",
                        frm: cur_frm
                })
        },
        make_debit_note_for_rejected_qty: function() {
                frappe.model.open_mapped_doc({
                        method: "core_erp.customizations.purchase_invoice.purchase_invoice.make_debit_note_for_reject",
                        frm: cur_frm
                })
        },
});
...
//	var item_fields_stock = ['warehouse_section', 'received_qty'];
