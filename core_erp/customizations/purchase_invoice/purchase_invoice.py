...
from erpnext.accounts.doctype.tax_withholding_category.tax_withholding_category import get_tax_withholding_details, get_tax_row_for_tds, get_advance_vouchers, is_valid_certificate
...
# 			for d in self.taxes
# 			if not d.tax_amount and d.account_head == tax_withholding_details.get("account_head")
# 		]/home/frappe/frappe-bench/apps/core_erp/core_erp/customizations/purchase_invoice
...
def get_expense_account(doctype, txt, searchfield, start, page_len, filters):
    ...
    return frappe.db.sql(
        ...
        where (tabAccount.account_type in ("Expense Account", "Fixed Asset", "Temporary", "Asset Received But Not Billed", "Capital Work in Progress"))
...
@frappe.whitelist()
def get_payment_schedule(doctype, docname):
    try:
        data = frappe.get_value(doctype, docname, 'payment_terms_template')
        return {"data": data}
    except Exception as e:
        frappe.log_error(frappe.get_traceback(), 'get_payment_schedule error')
        return {"error": str(e)}
